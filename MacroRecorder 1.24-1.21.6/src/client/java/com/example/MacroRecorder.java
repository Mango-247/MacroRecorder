...
public class MacroRecorder {
    ...
    public static void init() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            boolean stopPressed = stopRecordKey != -1 && InputUtil.isKeyPressed(window, stopRecordKey);
        ...
        HudRenderCallback.EVENT.register((drawContext, tickCounter) -> {
            if (isRecording && showIndicator && client.player != null) {
                ...
                drawContext.drawWrappedText(client.textRenderer, Text.literal(message), x, y, screenWidth, color, true);
            }
        });
    ...
    private static void moveItemToSlot(ItemStack desired, int targetSlot) {
        ...
        if (ItemStack.areItemsAndComponentsEqual(desired, current)) return;

        for (int i = 0; i < inv.size(); i++) {
            if (i == targetSlot) continue;
            ItemStack found = inv.getStack(i);
            if (ItemStack.areItemsAndComponentsEqual(desired, found)) {
